...
static const char *TAG = "bsp_mouse";

static void *bsp_mouse_handle = NULL;
static bsp_mouse_state_t mouse_state; // = { .x = 0, .y = 0, press = false };
...
esp_err_t bsp_mouse_init(bsp_mouse_conf_t *conf)
{
    ...
    bsp_mouse_handle = malloc(sizeof(bsp_mouse_conf_t));

    ESP_RETURN_ON_FALSE(NULL != bsp_mouse_handle, ESP_ERR_NO_MEM,
        TAG, "Failed create mouse handle");

    memcpy(bsp_mouse_handle, conf, sizeof(bsp_mouse_state_t));
...
{
    bsp_mouse_conf_t conf = {
        .init_ble_service = true,
    };
    return bsp_mouse_init(&conf);
}
...
esp_err_t bsp_mouse_get_state(bsp_mouse_state_t *state)
{
    ...
    memcpy(state, &mouse_state, sizeof(bsp_mouse_state_t));

    return ESP_OK;
}

esp_err_t bsp_mouse_event_handler(uint8_t *data, uint16_t length, esp_hid_usage_t usage)
{
