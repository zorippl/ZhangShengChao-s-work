...
void ui_mute_init(void)
{
    ...
    if (NULL == obj) {
        obj = lv_obj_create(lv_scr_act());                                      //获取活动画面指针
        lv_obj_add_flag(obj, LV_OBJ_FLAG_HIDDEN);                               //添加隐藏flag                    
        lv_obj_clear_flag(obj, LV_OBJ_FLAG_SCROLLABLE);                         //移除滚动flag
        lv_obj_set_size(obj, 180, 45);                                          //180, 45
        ...
        lv_label_set_text_static(lab, "Voice State");
